...
/***/ function(module, exports, __webpack_require__) {
	...
	var _events2 = __webpack_require__(13);

	var _events3 = __webpack_require__(21);

	var setAPIOrigin = __webpack_require__(22);
	var config = __webpack_require__(9);


	$(function () {
	  setAPIOrigin(location, config);
	  (0, _events.addGameEventHandlers)();
	  (0, _events2.addAuthEventHandlers)();
	  (0, _events3.addMenuHEventandlers)();
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var computerPlay = function computerPlay() {
	  ...
	  var cell = (0, _helpers.getRandomEmptyCell)();
	  ...
	  if (!target.text()) {
	    ...
	    target.text(_store2.default.currentPlay);

	    // update the store to be the next play
	    _store2.default.currentPlay = _store2.default.currentPlay === 'X' ? 'O' : 'X';

	    // turn off the click events until we have finished our api call
	    if (!(0, _helpers.determineOutcome)().over) {
	      _selectors2.default.gameBoard.cells.on('click', onTileClick);
	    }

	    // highlight the current play for the user so they know if the next
	    // play is an X or an O
	    _ui2.default.highlightCurrentPlay(_store2.default.currentPlay);

	    // turn clicks off and back on so we dont get duplicate events
	    resetTileClicks();

	    // get the data to send to the backend
	    var data = getUpdateGameData(target);

	    // save the new data to the database
	    _api2.default.updateGame(data)

	    // pass in the data and a callback to have the computer make a play
	    .then(function (data) {
	      return _ui2.default.onUpdateGameSuccess(data, computerPlay);
	    }).catch(_ui2.default.onUpdateGameFailure);
	  }
	};
	var getUpdateGameData = function getUpdateGameData(cellTarget) {
	  // get the current gameId from the store to save to database
	  var gameId = _store2.default.currentGame.id;

	  // see if the game is over so we can save it to db
	  var over = (0, _helpers.determineOutcome)().over;

	  // set the correct index and value for updating db
	  var index = cellTarget.data('cell');
	  var value = cellTarget.text();
	...
	var onTileClick = function onTileClick(event) {
	  ...
	  if (!target.text() && !_store2.default.computersTurn) {
	    // set the text to be whatever the current play is
	    target.text(_store2.default.currentPlay);
	    _store2.default.currentPlay = _store2.default.currentPlay === 'X' ? 'O' : 'X';

	    // turn off the click events until we have finished our api call
	    if ((0, _helpers.determineOutcome)().over || _store2.default.computersTurn) {
	      _selectors2.default.gameBoard.cells.off('click');
	    }
	    ...
	    // save the new data to the database
	    _api2.default.updateGame(data).then(function (data) {
	      return _ui2.default.onUpdateGameSuccess(data, computerPlay);
	    }).catch(_ui2.default.onUpdateGameFailure);
	  }
	};
	...
	var onNewGame = function onNewGame() {
	  ...
	  _store2.default.currentPlay = 'X';

	  // creates game in database and handles success/fail
	  _api2.default.createGame().done(resetTileClicks).then(_ui2.default.onNewGameSuccess).catch(_ui2.default.onNewGameFailure);
	};
	...
	var resetTileClicks = function resetTileClicks() {
	  _selectors2.default.gameBoard.cells.off('click', onTileClick);
	  _selectors2.default.gameBoard.cells.on('click', onTileClick);
	};

	var addGameEventHandlers = function addGameEventHandlers() {
	  // adds the click events for the gameboard
	  _selectors2.default.gameBoard.cells.on('click', onTileClick);
	  _selectors2.default.newGame.on('click', onNewGame);
	};

	module.exports = {
	  onNewGame: onNewGame,
	  addGameEventHandlers: addGameEventHandlers,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var createGame = function createGame() {
	  return $.ajax({
	    method: 'POST',
	    url: _config2.default.apiOrigin + '/games',
	    headers: {
	...
	// updates a game
	var updateGame = function updateGame(_ref) {
	  var cell = _ref.cell,
	      gameId = _ref.gameId,
	      over = _ref.over;
	  ...
	  return $.ajax({
	    method: 'PATCH',
	    url: _config2.default.apiOrigin + '/games/' + gameId,
	    headers: {
	      Authorization: 'Token token=' + _store2.default.user.token
	    },
	    data: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _helpers2 = __webpack_require__(11);

	var _constants = __webpack_require__(6);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var onNewGameSuccess = function onNewGameSuccess(_ref) {
	  var game = _ref.game;

	  // clear the "winner" alert
	  _selectors2.default.gameBoard.resultOverlay.slideUp();

	  // clear the screen
	  (0, _helpers2.hideAllAlerts)();
	  (0, _helpers2.hideAllContainersExcept)(_selectors2.default.gameBoard.container);

	  // clear the board of all text
	  _selectors2.default.gameBoard.cells.text('');

	  // highlight the correct turn
	  highlightCurrentPlay(_store2.default.currentPlay);

	  // show the game board if hidden
	  !_selectors2.default.gameBoard.container.is(':visible') && _selectors2.default.gameBoard.container.slideDown();

	  // set the current game to the store
	  _store2.default.currentGame = game;
	};
	var onNewGameFailure = function onNewGameFailure(data) {
	  // TODO update this
	};
	var onUpdateGameSuccess = function onUpdateGameSuccess(_ref2, callback) {
	  var game = _ref2.game;

	  // if the game is over
	  if (game.over) {
	    // its no longer the computers turn
	    _store2.default.computersTurn = false;

	    // figure out who the winner is from the response

	    var _determineOutcome = (0, _helpers.determineOutcome)(game.cells),
	        winner = _determineOutcome.winner;

	    if (winner === _constants.OUTCOME.DRAW) {
	      // set the text to be that it was a draw
	      _selectors2.default.gameBoard.resultOverlay.text('Draw');
	    } else {
	      // set the text to be who won
	      _selectors2.default.gameBoard.resultOverlay.text(winner + ' wins!');
	    }

	    // display the outcome to the user, then hide it so they can see
	    // how the other person won or how they won
	    _selectors2.default.gameBoard.resultOverlay.slideDown().delay(1000).slideUp();
	    ...
	    if (_selectors2.default.gameBoard.computerSwitch.prop('checked') && _store2.default.currentPlay === 'O') {
	      _store2.default.computersTurn = true;
	...
	var onShowStatisticsSuccess = function onShowStatisticsSuccess(_ref3) {
	  var games = _ref3.games;

	  // get the data from the user's games
	  var _getGameStatistics = (0, _helpers.getGameStatistics)(games),
	      wins = _getGameStatistics.wins,
	      losses = _getGameStatistics.losses,
	      draws = _getGameStatistics.draws,
	      winPercentage = _getGameStatistics.winPercentage;

	  // set the text for each of the relevant data pieces


	  _selectors2.default.gameStatistics.wins.text(wins);
	  _selectors2.default.gameStatistics.losses.text(losses);
	  _selectors2.default.gameStatistics.draws.text(draws);
	  _selectors2.default.gameStatistics.winPercentage.text(winPercentage(1) + '%');

	  // clear the screen
	  (0, _helpers2.hideAllContainersExcept)();

	  // show the game stats
	  _selectors2.default.gameStatistics.container.show();
	};
	var onShowStatisticsFailure = function onShowStatisticsFailure() {}
	// TODO handle this error


	// toggles which player is highlighted
	// this is used to show who's turn it is
	;var highlightCurrentPlay = function highlightCurrentPlay(currentPlay) {
	  var x = $('#player-x-title');
	  var o = $('#player-o-title');
	...
	module.exports = {
	  ...
	  onShowStatisticsSuccess: onShowStatisticsSuccess,
	  onShowStatisticsFailure: onShowStatisticsFailure,
	  highlightCurrentPlay: highlightCurrentPlay
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var hideAllContainersExcept = function hideAllContainersExcept(ignoreElement) {
	  var containers = [$('.main-header'), $('#winner-header'), _selectors2.default.signUp.container, _selectors2.default.signIn.container, _selectors2.default.changePassword.container, _selectors4.default.gameBoard.container, _selectors4.default.gameStatistics.container];
	...
	// TODO clean this up
	var hideAllAlerts = function hideAllAlerts(isAnimated) {
	  var alerts = [];
	  alerts.push(_selectors2.default.alerts);
	  alerts.push(_selectors4.default.alerts);
	  alerts.forEach(function (alertSet) {
	    for (var alert in alertSet) {
	      if (isAnimated) {
	...
	module.exports = {
	  hideAllContainersExcept: hideAllContainersExcept,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _getFormFields = __webpack_require__(14);
	...
	var onSignUp = function onSignUp(event) {
	  ...
	  if (email && password && passwordConfirmation) {
	    // show loader
	    (0, _helpers2.showFormLoader)(_selectors2.default.signUp);
	    ...
	    (0, _animations.showAlert)(_selectors2.default.alerts.signUpEmpty);
	  }
	};
	var onSignIn = function onSignIn(event) {
	  ...
	  if (data.credentials.email && data.credentials.password) {
	    // show loader
	    (0, _helpers2.showFormLoader)(_selectors2.default.signIn);
	    ...
	    (0, _animations.showAlert)(_selectors2.default.alerts.signInEmpty);
	  }
	};
	var onSignOut = function onSignOut(event) {
	  ...
	  if (_store.user) {
	    ...
	    _api2.default.signOut(_store.user).then(_ui2.default.signOutSuccess).catch(_ui2.default.signOutFailure);
	  } else {
	...
	var onChangePassword = function onChangePassword(event) {
	  ...
	  if (data.passwords.old && data.passwords.new) {
	    // show loader
	    (0, _helpers2.showFormLoader)(_selectors2.default.changePassword);

	    // send the data to the backend and handle success/fail
	    _api2.default.changePassword(data, _store.user).then(_ui2.default.changePasswordSuccess).catch(_ui2.default.changePasswordFailure);
	  } else {
	    // if they havent filled in the form, alert them
	    (0, _animations.showAlert)(_selectors2.default.alerts.changePasswordEmpty);
	  }
	};
	...
	var onShowSignUp = function onShowSignUp(event) {
	  ...
	  (0, _helpers.hideAllAlerts)();

	  // show the sign up form
	  _selectors2.default.signUp.container.show();

	  // hide the sign in form
	  _selectors2.default.signIn.container.hide();
	};
	...
	var onShowSignIn = function onShowSignIn(event) {
	  ...
	  _selectors2.default.signIn.container.show();

	  // hide the sign up form
	  _selectors2.default.signUp.container.hide();
	};

	// add all the event handlers to their respective selectors
	var addAuthEventHandlers = function addAuthEventHandlers() {
	  _selectors2.default.signUp.form.on('submit', onSignUp);
	  _selectors2.default.signIn.form.on('submit', onSignIn);
	  _selectors2.default.changePassword.form.on('submit', onChangePassword);
	  _selectors2.default.signIn.signUpLink.on('click', onShowSignUp);
	  _selectors2.default.signUp.signInLink.on('click', onShowSignIn);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var changePassword = function changePassword(data, user) {
	  return $.ajax({
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _selectors5 = __webpack_require__(7);
	...
	var signUpSuccess = function signUpSuccess(data) {
	  ...
	  (0, _helpers2.hideFormLoader)(_selectors2.default.signUp);

	  // get the credentials the user entered when signing up
	  // so we can sign the user in automatically
	  var credentials = {
	    email: _selectors2.default.signUp.email.val(),
	    password: _selectors2.default.signUp.password.val()
	  };
	  ...
	  _selectors2.default.signUp.email.val('');
	  _selectors2.default.signUp.password.val('');
	  _selectors2.default.signUp.passwordConfirmation.val('');
	  _selectors2.default.signUp.email.parent('div').toggleClass('is-dirty');
	  _selectors2.default.signUp.password.parent('div').toggleClass('is-dirty');
	  _selectors2.default.signUp.passwordConfirmation.parent('div').toggleClass('is-dirty');
	};
	var signUpFailure = function signUpFailure() {
	  ...
	  (0, _animations.showAlert)(_selectors2.default.alerts.signUpFailure);
	};
	var signInSuccess = function signInSuccess(_ref) {
	  ...
	  // clear containers
	  (0, _helpers.hideAllContainersExcept)();

	  // hide the loader and put the button back for if the user signs out again
	  (0, _helpers2.hideFormLoader)(_selectors2.default.signIn);

	  // show the user's email in the header
	  _selectors4.default.menu.email.text(user.email);

	  // show the header now that the user is signed in
	  _selectors4.default.menu.container.slideDown('fast');

	  // clear/reset the sign in form in case the user gets back there somehow
	  _selectors2.default.signIn.email.val('');
	  _selectors2.default.signIn.password.val('');
	  _selectors2.default.signIn.email.parent('div').toggleClass('is-dirty');
	  _selectors2.default.signIn.password.parent('div').toggleClass('is-dirty');
	...
	var signInFailure = function signInFailure() {
	  ...
	  (0, _animations.showAlert)(_selectors2.default.alerts.signInFailure);
	};
	var signOutSuccess = function signOutSuccess() {
	  ...
	  _selectors6.default.gameBoard.resultOverlay.hide();

	  // hide the navbar because the user is logged out
	  _selectors4.default.menu.container.hide();
	  ...
	  (0, _animations.showTemporaryAlert)(_selectors2.default.alerts.signOutSuccess);

	  // clear out the store so another user can sign in
	  _store2.default.user = null;
	};
	...
	var changePasswordSuccess = function changePasswordSuccess(data) {
	  ...
	  (0, _helpers2.hideFormLoader)(_selectors2.default.changePassword);

	  // let the user know the password was changed
	  (0, _animations.showAlert)(_selectors2.default.alerts.changePasswordSuccess);
	};
	var changePasswordFailure = function changePasswordFailure() {
	  ...
	  (0, _animations.showAlert)(_selectors2.default.alerts.changePasswordFailure);
	};
	module.exports = {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var showAlert = function showAlert(element) {
	  (0, _helpers.hideAllAlerts)();
	  element.slideToggle();
	};
	var showTemporaryAlert = function showTemporaryAlert(element) {
	  (0, _helpers.hideAllAlerts)();
...
/***/ function(module, exports) {
	...
	var showFormLoader = function showFormLoader(form) {
	  form.container.find('button').slideToggle(0, function () {
	    form.loader.slideToggle();
	  });
	};
	var hideFormLoader = function hideFormLoader(form) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _selectors2 = __webpack_require__(7);
	...
	var _selectors4 = __webpack_require__(12);

	var _selectors5 = _interopRequireDefault(_selectors4);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var onShowStatistics = function onShowStatistics(event) {
	  // get all the user's games so we can show them their stats
	  _api2.default.gamesIndex().then(_ui2.default.onShowStatisticsSuccess).catch(_ui2.default.onShowStatisticsFailure);
	};
	...
	var onShowChangePassword = function onShowChangePassword(event) {
	  ...
	  _selectors5.default.changePassword.container.show();
	};
	var onShowGameBoard = function onShowGameBoard(event) {
	  ...
	  // show the game board
	  _selectors3.default.gameBoard.container.show();
	};
	...
	var addMenuHEventandlers = function addMenuHEventandlers() {
	  $('.header-drawer button').on('click', onMenuItemClick);
	  _selectors.menu.signOut.on('click', _events2.default.onSignOut);
	  _selectors.menu.gameOptions.on('click', onShowGameBoard);
	  _selectors.menu.changePassword.on('click', onShowChangePassword);
	  _selectors.menu.gameStats.on('click', onShowStatistics);
	};

	module.exports = {
	  addMenuHEventandlers: addMenuHEventandlers
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var parseNestedQuery = __webpack_require__(23);
